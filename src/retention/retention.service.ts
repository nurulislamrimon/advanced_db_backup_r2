...
export class RetentionService {
  ...
  async applyRetentionPolicy(dryRun = false): Promise<RetentionResult> {
    ...
    this.logger.log(`Retention: keep=${toKeep.length}, delete=${toDelete.length}`);
  ...
  private parseBackupDates(keys: string[]): BackupInfo[] {
    ...
    return keys
      .map((key) => {
        ...
        if (!match) {
          this.logger.warn(`Could not parse backup key: ${key}`);
          return null;
        }
        ...
        if (isNaN(date.getTime())) {
          this.logger.warn(`Invalid date in backup key: ${key}`);
          return null;
        }
